...
**GitHubのコード調査に、根拠の検証とAPIの実行予算を組み込む。**
...
既存ファイルの変更には、そのファイル自身の根拠行を必須とします。実評価で別ファイルの引用だけが付いた変更案を拒否したことを受け、モデルの出力形式でも「変更先内の根拠」と「補足の根拠」を分けました。行や理由をサーバーで埋め合わせることはせず、未読ファイルや不正な行範囲は失敗として扱います。

この検証で確認できるのは、引用先の存在や取得したコードとの対応です。その行が提案を意味的に裏付けるかは、別途レビューが必要です。

### 調査と最終出力の両方に予算を設ける

調査は最大18ファイル、合計160,000バイト、ツール28回、モデル12ターン、全体90秒に制限しました。最後の1ターンを計画出力に残し、出力段階では追加の調査ツールを停止します。1回のモデル要求にも45秒の期限があります。

検索先にディレクトリを指定するような訂正可能な誤りには、合計2回まで訂正方法を返します。誤指定も呼出上限に含め、無制限の再試行を防ぎます。取得失敗・時間切れ・未完了応答を完成した計画として表示しません。

既定モデルの推論・文章量をlow、推論込みの出力上限を4,000トークンに調整しました。各要求の使用量は取得できた分を合算し、全応答取得・一部取得・未取得を区別します。回数やトークンの上限は、課金額を固定する保証ではありません。
...
1回の観測値であり、時間短縮の効果や一般的な成功率は判断できません。その後、変更先の根拠を必須にする出力形式と、失敗候補を特定する診断情報を追加し、モックで検証しました。**この最終修正後の実生成と、計画の意味的品質は未検証です。**
